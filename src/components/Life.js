...
export default function WeeksOfLife() {
            ...
            {Array.from({ length: totalRows }).map((_, row) => (
              <div key={row} className="flex justify-center mb-1">
                {Array.from({ length: weeksPerRow }).map((_, col) => {
                  ...
                  let cellClass = "w-2.5 h-2.5 m-0.5 rounded-sm transition-all duration-100 cursor-pointer ";
                  if (isPast) {
  ...
  const StatCard = ({ title, icon, children }) => (
    <div className={`p-6 rounded-2xl shadow-md border hover:shadow-lg transition-all duration-300 ${
      isDarkMode 
  ...
  const StatItem = ({ label, value, unit }) => (
    <div className="flex justify-between items-baseline">
      ...
      <span className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
        {value} {unit && <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{unit}</span>}
      </span>
    </div>
  );
      ...
      <div className="mt-8 space-y-6">
        ...
        <div className={`p-6 rounded-2xl border ${
          ...
          <div className={`text-sm ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}>
            ...
            <p className="mt-2">How will you use your remaining {getFormattedNumber(stats.weeksRemaining)} weeks?</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 p-4 pt-8 md:pt-12 ${
      isDarkMode 
      ...
      {showTooltip && hoverWeek !== null && (
        <div 
          className={`fixed z-50 p-2 rounded-lg shadow-lg text-sm pointer-events-none transition-opacity duration-200 ${
            isDarkMode 
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {step === 1 ? (
          <div className={`p-6 md:p-8 rounded-2xl shadow-md border max-w-md mx-auto ${
            isDarkMode 
              ? 'bg-gray-800 border-gray-700' 
          ...
          <>
            ...
            <div className="mt-8 flex justify-center gap-4">
              <button
                onClick={handleShare}
                disabled={isSharing}
                className={`px-6 py-2 border rounded-lg transition-colors shadow-sm flex items-center gap-2 ${
                  isDarkMode 
